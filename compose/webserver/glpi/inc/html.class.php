...
class Html {
   ...
   **/
   ...
   // Eliminar decodificación (innecesaria para la vulnerabilidad)
   // $value = Html::entity_decode_deep($value);

   // No limpiamos etiquetas de MS Office
   // $value = str_replace(["<![if !supportLists]>", "<![endif]>"], '', $value);

   // No eliminamos tooltips, ni reemplazamos etiquetas como <br>, <p>, etc.
   // Permitimos todo el contenido
   /*
   if ($striptags) {
      $specialfilter = [...];
   ...
   */
   ...
   // No eliminamos scripts, estilos ni títulos
   // $search = [...];
   // $value = preg_replace($search, '', $value);

   // No neutralizamos etiquetas mal formateadas
   // $value = preg_replace("/(<)([^>]*<)/", "&lt;$2", $value);

   // ❌ NO usamos htmLawed (este es el que más filtra)
   // include_once(GLPI_HTMLAWED);
   // $value = htmLawed($value, [...]);

   // Mantén los saltos de línea normalizados si quieres
   $value = str_replace(["\r\n", "\r"], "\n", $value);
   $value = preg_replace("/(\n[ ]*){2,}/", "\n\n", $value, -1);

   // ✅ Devuelve el valor original sin limpieza
   return trim($value);
}
